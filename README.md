...
| `SENDGRID_EVENT_WEBHOOK_PUBLIC_KEY` | server | optional, required for signed `/api/webhooks/sendgrid` ingestion |
| `SENDGRID_EVENT_EMAIL_HASH_SALT` | server | optional salt before hashing webhook recipient emails |
...
A human is in the loop at every step. The default SendGrid sync creates a Design / Dynamic
Template draft; the guarded Single Send API requires explicit confirmation before scheduling, and
the performance loop only learns from sends that have been linked back to Studio.

### SendGrid performance loop

1. Sync or create SendGrid assets from Output, then link the Single Send id when the campaign is scheduled.
2. Run **Performance → Sync SendGrid stats** to pull Single Send stats and click links into `send_history`.
3. Optional real-time events: configure SendGrid's signed Event Webhook to `POST /api/webhooks/sendgrid`, set `SENDGRID_EVENT_WEBHOOK_PUBLIC_KEY`, and apply `supabase/migrations/0005_send_events.sql`. Recipient emails are stored only as salted SHA-256 hashes.
4. The next generation for that brand receives a compact performance signal: winning levers to lean into and lagging levers to rotate away.
