...
const ELEMENT_AB_RULES = `ELEMENT A/B OPTION RULES:
- Besides subject/preheader options, generate A/B options for banner, each segment body, and every product image brief.
- banner.options must contain exactly 2 options labeled A and B. Each option includes copy, main/sub image direction, trust-booster, emergency, and review texts.
- body_options must contain exactly 2 options for "base" and for each selected segment key. Each option includes body, ps, placement_note.
- product.image_options must contain exactly 2 options for every product. Product image options are image briefs with overlay_copy, alt_text, and notes.
- These A/B element options are generated by the selected AI model for the current option; model_hint should identify the chosen provider/lens.`;
...
export function buildSystemPrompt(
  ...
  const productSlots = products
    .map(
      ...
      "image_options": [
        {"label": "A", "model_hint": "<selected AI model/lens>", "main_image": "<product image A direction>", "sub_image": "<close-up/supporting image A>", "overlay_copy": "<all text/CTA embedded inside image>", "alt_text": "<descriptive alt>", "notes": "<designer/dev notes>"},
        {"label": "B", "model_hint": "<selected AI model/lens>", "main_image": "<product image B direction>", "sub_image": "<close-up/supporting image B>", "overlay_copy": "<all text/CTA embedded inside image>", "alt_text": "<descriptive alt>", "notes": "<designer/dev notes>"}
      ],
...

{
  ...
  "banner": {
    ...
    "cta": "<2-4 word CTA>",
    "options": [
      {"label": "A", "model_hint": "<selected AI model/lens>", "main_text_1": "", "main_text_2": "", "sub_text_1": "", "sub_text_2": "", "cta": "", "review_texts": [""], "main_image": "", "sub_image": "", "trust_booster": "", "emergency": "", "image_guidance": ""},
      {"label": "B", "model_hint": "<selected AI model/lens>", "main_text_1": "", "main_text_2": "", "sub_text_1": "", "sub_text_2": "", "cta": "", "review_texts": [""], "main_image": "", "sub_image": "", "trust_booster": "", "emergency": "", "image_guidance": ""}
    ]
  },
  "body": {
    "base": "${bodySchemaHint}",
  ...
  "body_options": {
    "base": [
      {"label": "A", "model_hint": "<selected AI model/lens>", "body": "<body option A>", "ps": "<10-15 words>", "placement_note": "<where this body works best>"},
      {"label": "B", "model_hint": "<selected AI model/lens>", "body": "<body option B>", "ps": "<10-15 words>", "placement_note": "<where this body works best>"}
    ],
    ${campaign.segments
      .map(
        (id) => `"${segJsonKey(id)}": [
      {"label": "A", "model_hint": "<selected AI model/lens>", "body": "<segment ${id} body option A>", "ps": "<10-15 words>", "placement_note": "<where this body works best>"},
      {"label": "B", "model_hint": "<selected AI model/lens>", "body": "<segment ${id} body option B>", "ps": "<10-15 words>", "placement_note": "<where this body works best>"}
    ]`
      )
      .join(",\n    ")}
  },
...
export function validateBrief(brief: GenBrief, campaign: Campaign, products: Product[] = []): GenBrief {
  ...
  if (!Array.isArray(banner.options) || banner.options.length < 2) {
    addFlag(brief, "warn", "Banner needs A/B options");
  }
  ...
  const bodyOptions = brief.body_options || {};
  ["base", ...campaign.segments.map(segJsonKey)].forEach((key) => {
    const opts = bodyOptions[key] || [];
    if (opts.length < 2) addFlag(brief, "warn", `${key} body needs A/B options`);
    opts.forEach((o, i) => {
      if (!o.body) addFlag(brief, "warn", `${key} body option ${i + 1} missing body`);
      if (!o.ps) addFlag(brief, "warn", `${key} body option ${i + 1} missing P.S.`);
      if (o.body && wordCount(o.body) > 150) addFlag(brief, "warn", `${key} body option ${i + 1} over 150 words`);
      const psCount = wordCount(o.ps || "");
      if (o.ps && (psCount < 10 || psCount > 15)) addFlag(brief, "warn", `${key} body option ${i + 1} P.S. should be 10-15 words`);
    });
  });
  ...
  prods.forEach((p, i) => {
    ...
    if (!Array.isArray(p.image_options) || p.image_options.length < 2) addFlag(brief, "warn", `Product ${i + 1} image needs A/B options`);
    (p.image_options || []).forEach((o, j) => {
      if (!o.overlay_copy) addFlag(brief, "warn", `Product ${i + 1} image option ${j + 1} missing overlay copy`);
      if (!o.alt_text) addFlag(brief, "warn", `Product ${i + 1} image option ${j + 1} missing alt text`);
    });
    (p.usps || []).forEach((u, j) => {
